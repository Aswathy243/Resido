...
app.get("/attendance/:month", async (req, res) => {
  try {
    ...
    const attendance = await Register.aggregate([
      {
        $match: { role: { $nin: ["Resident", "Admin"] } }, // Exclude Resident, Admin & Treasurer
      },
      {
        $project: {
          fullName: 1,
...
app.post("/assign-task", async (req, res) => {
  try {
    ...
    if (staff.locked) {
      return res.status(400).json({ error: "This staff member has a pending task and is locked!" });
    }

    // Assign the task and lock the staff
    staff.tasks.push({ task, dueDate });
    staff.locked = true;
    await staff.save();

    res.status(200).json({ message: "Task assigned successfully and staff locked!" });
  } catch (error) {
...
app.post("/complete-task", async (req, res) => {
  try {
    ...
    // Remove all tasks and unlock the staff
    staff.tasks = [];
    staff.locked = false;
    await staff.save();

    res.status(200).json({ message: "Task completed and staff unlocked!" });
  } catch (error) {

...
const features = [
  { id: "1", name: "Visitor Log", icon: "clipboard", navigateTo: "visres" },
  { id: "2", name: "LPG Bills", icon: "flame", navigateTo: "MeterReadingScreen.js" },
  { id: "3", name: "Facility booking", icon: "calendar", navigateTo: "Hallres" },
];

const HomeScreen = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { flatNumber, residentId, residentName } = route.params || {}; // Retrieve params from login
        ...
        renderItem={({ item }) => (
          <View style={styles.featureWrapper}>
            <TouchableOpacity 
              ...
              onPress={() => {
                if (item.navigateTo === "visres") {
                  navigation.navigate("visres", { flatNumber });
                } else if (item.navigateTo === "Hallres") {
                  navigation.navigate("Hallres", { 
                    flatNumber,
                    residentId,
                    residentName,
                  });
                } else if (item.navigateTo === "MeterReadingScreen.js") {
                  ...
                  navigation.navigate(item.navigateTo || item.name.replace(/\s/g, ""));
                }
              }}
            >
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              if (tab.name === "logout") {
                ...
                console.log("🔍 Navigating to Profile with userId:", residentId);
                navigation.navigate("profile", {residentId });
              }
              else {
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#F5F5F5", padding: 20, paddingTop: height * 0.12 },
  headerSection: {
    height: height * 0.29,
  ...
  menuIcon: { position: "absolute", left: 20, top: 40, zIndex: 2 },
  headerText: { top: 15, fontSize: 25, fontWeight: "bold", letterSpacing: 1, color: "white" },
  searchContainer: { 
    flexDirection: "row",
    alignItems: "center",
  ...
  searchInput: { flex: 1, fontSize: 14, paddingHorizontal: 7 },
  featureWrapper: { flex: 1, alignItems: "center", marginVertical: 20 },
  featureCard: { 
  ...
  featureText: { marginTop: 10, color: "#333", fontWeight: "bold", fontSize: 15, textAlign: "center" },
  bottomNav: { 
    flexDirection: "row", 
  ...
  navText: { fontSize: 12, color: "grey", marginTop: 4 },
  emptyContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
